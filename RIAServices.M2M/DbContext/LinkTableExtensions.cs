namespace RIAServices.M2M.DbContext
{
    ...
    public static class LinkTableExtensions
    {
        ...
        public static void LoadM2M<TObject1, TObject2, TLinkTable>(this DbContext context, TObject1 object1, TObject2 object2)
        ...
        public static TObject1 FetchObject1<TObject1, TObject2>(
            this LinkTable<TObject1, TObject2> linkTableEntity, ChangeSet changeSet, DbContext dbContext)
            where TObject1 : class where TObject2 : class
        {
            PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(linkTableEntity.GetType());
            PropertyDescriptor navProp = properties["Object1"];
        ...
        {
            PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(linkTableEntity.GetType());
            PropertyDescriptor navProp = properties["Object2"];
            AssociationAttribute associationAttribute =
                navProp.Attributes.OfType<AssociationAttribute>().SingleOrDefault();
        ...
        {
            object[] thisKeyValues = MakeKeyValues(linkTableEntity, association.ThisKeyMembers);
            object[] otherKeyValues = MakeKeyValues(entity, association.OtherKeyMembers);
            for(int i = 0; i < thisKeyValues.Count(); i++)
            {
